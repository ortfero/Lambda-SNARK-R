...
This specification defines the Zero-Knowledge (ZK) extension for ΛSNARK-R prover. The goal is to achieve **honest-verifier zero-knowledge (HVZK)** by adding polynomial blinding to commitments and openings, ensuring that proofs reveal nothing about the witness beyond the statement's validity.

**Approach**: Polynomial blinding with random masking polynomial r(X)

**Key Properties**:
- ✅ Zero-Knowledge: Simulator indistinguishable from real prover
- ✅ Soundness preserved: ε ≤ 2^-44 (unchanged)
- ✅ Completeness preserved: Valid proofs verify (100%)
- ✅ Non-interactive: Via Fiat-Shamir transform (already implemented)
...
**Prover Algorithm**:
```
1. Encode witness: f(X) = Σ z_i·X^i
2. Commit: C = Commit(f(X), seed)
3. Challenge: α = H(public_inputs || C)
4. Opening: y = f(α)
5. Proof: π = (C, α, y, witness)
```

**Problem**: Opening reveals `y = f(α)`, which may leak information about witness `z`.

**Example Leakage**:
- Witness: z = [z₀, z₁, ..., zₙ]
- Evaluation: y = f(α) = z₀ + z₁·α + z₂·α² + ... + zₙ·αⁿ
...
**Blinding Polynomial**:
```
r(X) = r₀ + r₁·X + r₂·X² + ... + r_d·X^d
where r_i ← U(F_q) (uniform random from field)
...
5. Derive Fiat-Shamir challenge:
   α = H(public_inputs || C)  (mod q)
   
6. Compute blinded evaluation:
   y' = f'(α) = f(α) + r(α)  (mod q)
   
7. Generate opening proof:
   opening = Opening(y', witness=[seed, f'₀, f'₁, ..., f'ₙ])
...
5. Generate opening:
   opening = Opening(y', witness=[random_seed, f'₀, ..., f'ₙ])
...
```
π_real ~ (Commit(f(X) + r(X)), α, f(α) + r(α), opening)
where r(X) ← U(F_q^{n+1})
```

**Simulated Proof Distribution**:
```
π_sim ~ (Commit(f'(X)), α, f'(α), opening)
where f'(X) ← U(F_q^{n+1})
```
...
- Therefore: f(X) + r(X) ≡ f'(X) (distribution equivalence)

**Distinguisher Advantage**:
```
Adv_ZK(A) = |Pr[A(π_real) = 1] - Pr[A(π_sim) = 1]|
           ≤ Adv_LWE(B) + negl(λ)
           ≈ 2^-128 (for λ=128)
```
...
**Argument**:
- Verifier checks f'(α) = y' (blinded polynomial evaluation)
...
- Must provide f'(X) such that f'(α) = y' for random α
- Probability of success ≤ 1/q ≈ 2^-44 (polynomial degree bound)
...
- Challenge α: deterministic from C (no info)
- Evaluation y': blinded by r(α) (uniformly random)
...
**Temporary Storage**:
- Blinding polynomial r(X): (n+1)×8 bytes ≈ 40 bytes for n=4
- Blinded polynomial f'(X): (n+1)×8 bytes ≈ 40 bytes
...
1. **Random Polynomial Generation**
   ...
   - Determinism: Same seed → same polynomial

2. **Polynomial Addition**
   - Correctness: (f + r)(α) = f(α) + r(α)
...
4. **ZK Proof Generation**
   - Valid witness → valid ZK proof
   - Different blindings → different proofs (same witness)
   - Deterministic blinding → deterministic proof

5. **ZK Proof Verification**
   - Valid ZK proof → accepts
   - Invalid evaluation → rejects
   - Soundness: forged proof → rejects

6. **Simulator Tests**
   - Simulator produces valid-looking proofs
   - Distinguisher advantage ≤ negligible (statistical test)

### Security Tests

7. **Witness Hiding**
   - Extract witness from proof → fails (brute-force)
   - Information leakage: I(z ; π) ≈ 0 (empirical)

8. **Soundness Preservation**
   - Invalid witness → proof rejects (with/without ZK)

9. **Completeness**
   - All valid witnesses → proofs verify (100% rate)
...
3. **Optimized Randomness**
   - Use PRG to expand short seed → long blinding
...
- ✅ **Zero-Knowledge**: Simulator indistinguishable (Adv ≤ 2^-128)
- ✅ **Soundness**: Preserved (ε ≤ 2^-44)
- ✅ **Completeness**: Preserved (100%)
- ✅ **Performance**: <1% overhead
- ✅ **Simplicity**: Minimal API changes
