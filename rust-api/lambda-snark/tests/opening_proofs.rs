...
use lambda_snark::{
    generate_opening, verify_opening, Challenge, Commitment, LweContext, Opening, Polynomial,
};
...
fn test_opening_pipeline_tv1() {
    ...
    assert_eq!(
        opening.evaluation(),
        expected_eval,
        "Evaluation should match polynomial.evaluate(α)"
    );
}
...
fn test_opening_deterministic() {
    ...
    assert_eq!(
        opening1.evaluation(),
        opening2.evaluation(),
        "Same inputs should produce same evaluation"
    );
    assert_eq!(
        opening1.witness(),
        opening2.witness(),
        "Same inputs should produce same witness"
    );
}
...
fn test_opening_multiple_commitments() {
    ...
    assert!(
        verify_opening(&comm1, alpha, &opening1, TEST_MODULUS),
        "Opening 1 should verify"
    );
    assert!(
        verify_opening(&comm2, alpha, &opening2, TEST_MODULUS),
        "Opening 2 should verify"
    );
...
fn test_opening_consistency_with_prover_pipeline() {
    ...
    assert_eq!(
        opening.evaluation(),
        polynomial.evaluate(alpha),
        "Opening evaluation should match polynomial evaluation"
    );

    assert!(
        verify_opening(&commitment, alpha, &opening, TEST_MODULUS),
