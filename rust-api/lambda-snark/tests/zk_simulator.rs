...
use lambda_snark::{simulate_proof, prove_zk, LweContext, Params, Profile, SecurityLevel};
...
fn test_simulator_vs_real_structure() {
    ...
    let real_proof = prove_zk(&witness, &public_inputs, &ctx, TEST_MODULUS, 0x1234, Some(42))
...
fn test_real_vs_sim_challenge_distribution() {
    ...
    for i in 0..num_samples {
        // Real proof
        let real_proof = prove_zk(&witness, &public_inputs, &ctx, TEST_MODULUS, 0x1000 + i, Some(i))
            .expect("Failed to generate real proof");
        real_challenges.push(real_proof.challenge().alpha().value());
        
        // Simulated proof
        let sim_proof = simulate_proof(3, &public_inputs, &ctx, TEST_MODULUS, 0x2000 + i, Some(i + 100))
            .expect("Failed to simulate proof");
    ...
    assert!(real_unique.len() >= (num_samples - 1) as usize, "Real challenges should be diverse");
    assert!(sim_unique.len() >= (num_samples - 1) as usize, "Simulated challenges should be diverse");
...
fn test_distinguisher_challenge_range() {
    ...
    for i in 0..num_samples {
        let real = prove_zk(&witness, &public_inputs, &ctx, TEST_MODULUS, 0x1000 + i, Some(i))
            .expect("Failed to generate real proof");
        real_challenges.push(real.challenge().alpha().value());
        
        let sim = simulate_proof(3, &public_inputs, &ctx, TEST_MODULUS, 0x2000 + i, Some(i + 50))
            .expect("Failed to simulate proof");
    ...
    let real_mean: f64 = real_challenges.iter().map(|&x| x as f64).sum::<f64>() / num_samples as f64;
    let sim_mean: f64 = sim_challenges.iter().map(|&x| x as f64).sum::<f64>() / num_samples as f64;
    let expected_mean = TEST_MODULUS as f64 / 2.0;
    
    println!("Real mean: {:.2e}, Sim mean: {:.2e}, Expected: {:.2e}", real_mean, sim_mean, expected_mean);
...
fn test_distinguisher_evaluation_range() {
    ...
    println!("Real evaluations: {} unique out of {}", real_unique.len(), num_samples);
    println!("Sim evaluations: {} unique out of {}", sim_unique.len(), num_samples);
    
    // Should have high diversity (most values unique)
    assert!(real_unique.len() >= (num_samples - 2) as usize, "Real evaluations should be diverse");
    assert!(sim_unique.len() >= (num_samples - 2) as usize, "Sim evaluations should be diverse");
}
...
fn test_practical_distinguisher() {
    ...
    for i in 0..num_samples {
        if i % 2 == 0 {
            // Real proof
            let proof = prove_zk(&witness, &public_inputs, &ctx, TEST_MODULUS, 0x1000 + i, Some(i))
                .expect("Failed to generate real proof");
            proofs.push(proof);
            labels.push(true);
        } else {
            // Simulated proof
            let proof = simulate_proof(3, &public_inputs, &ctx, TEST_MODULUS, 0x2000 + i, Some(i + 100))
                .expect("Failed to simulate proof");
            proofs.push(proof);
            labels.push(false);
        }
    ...
    println!("Distinguisher accuracy: {:.2}% ({}/{})", accuracy * 100.0, correct_guesses, num_samples);
...
fn test_simulator_different_degrees() {
    ...
    for degree in [0, 1, 3, 10, 20] {
        let sim_proof = simulate_proof(degree, &public_inputs, &ctx, TEST_MODULUS, 0x1234, Some(42))
