...
use lambda_snark::{Challenge, Commitment, LweContext, Params, Polynomial, Profile, SecurityLevel};
...
fn test_full_pipeline_tv1() {
    ...
    let seed = 0xDEADBEEF;
    let commitment =
        Commitment::new(&ctx, polynomial.coefficients(), seed).expect("Commitment failed");
    ...
    assert!(
        challenge.alpha().value() < TEST_MODULUS,
        "Challenge should be in F_q"
    );

    // Verify evaluation is computable
    assert!(
        evaluation.value() < TEST_MODULUS,
        "Evaluation should be in F_q"
    );
