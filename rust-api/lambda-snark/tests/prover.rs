...
fn test_prove_deterministic() {
    ...
    let valid1 = verify_opening(
        &proof1.commitment,
        proof1.challenge.alpha(),
        &proof1.opening,
        TEST_MODULUS,
    );
    let valid2 = verify_opening(
        &proof2.commitment,
        proof2.challenge.alpha(),
        &proof2.opening,
        TEST_MODULUS,
    );
...
fn test_proof_size_measurement() {
    ...
    println!(
        "Commitment: {} bytes ({:.1} KB)",
        comm_bytes.len(),
        comm_bytes.len() as f64 / 1024.0
    );
    ...
    assert!(
        comm_bytes.len() > 65000 && comm_bytes.len() < 66000,
...
fn test_proof_generation_time() {
    ...
    assert!(
        avg_time.as_secs() < 1,
        "Average proof time should be < 1s, got {:?}",
        avg_time
    );
}
...
fn test_proof_components_consistency() {
    ...
    assert_eq!(
        proof.opening.evaluation(),
        opening_manual.evaluation(),
