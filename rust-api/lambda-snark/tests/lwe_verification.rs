...
use lambda_snark::{
    ...
    generate_opening, verify_opening_with_context,
    LweContext,
};
...
fn test_lwe_verification_wrong_randomness() {
    ...
    let valid = verify_opening_with_context(&commitment, alpha, &opening, TEST_MODULUS, &ctx);
    assert!(!valid, "Opening with wrong randomness should be rejected by LWE verification");
}
...
fn test_lwe_verification_wrong_polynomial() {
    ...
    assert!(!valid, "Opening for different polynomial should be rejected by LWE verification");
}
...
fn test_lwe_verification_deterministic() {
    ...
    for i in 0..5 {
        let valid = verify_opening_with_context(&commitment, alpha, &opening, TEST_MODULUS, &ctx);
        assert!(valid, "LWE verification should be deterministic (iteration {})", i);
    }
}
