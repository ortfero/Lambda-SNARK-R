...
use lambda_snark::{Polynomial, Commitment, LweContext, Params, Profile, SecurityLevel, Field};
...
fn test_commitment_reproducible_structure() {
    ...
    assert_eq!(comm1.as_bytes().len(), comm2.as_bytes().len(), 
...
fn test_commitment_distinct() {
    ...
    assert_ne!(comm1.as_bytes(), comm2.as_bytes(), "Different polynomials should produce different commitments");
}
...
fn test_tv1_polynomial_commitment() {
    ...
    for (i, &expected) in witness.iter().enumerate() {
        assert_eq!(p.coeff(i).unwrap().value(), expected, "Coefficient mismatch at index {}", i);
    }
    ...
    println!("TV-1 commitment size: {} u64 elements", comm.as_bytes().len());
}
...
fn test_tv2_polynomial_commitment() {
    ...
    assert!(!comm.as_bytes().is_empty());
    println!("TV-2 commitment size: {} u64 elements", comm.as_bytes().len());
}
