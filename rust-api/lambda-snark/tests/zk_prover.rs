...
fn test_different_blindings_different_commitments() {
    ...
    assert_ne!(
        commitment1_bytes, commitment2_bytes,
...
fn test_different_blindings_different_challenges() {
    ...
    assert_ne!(
        proof1.challenge().alpha().value(),
        proof2.challenge().alpha().value(),
...
fn test_zk_completeness_multiple_witnesses() {
    ...
    for (witness, public_inputs) in test_cases {
        ...
        assert!(
            verify_simple(&proof, &public_inputs, TEST_MODULUS),
            "Proof should verify for witness {:?}",
            witness
        );
    }
}
...
fn test_zk_proof_performance() {
    ...
    assert!(
        ...
        "ZK proof generation took {:?} (expected < 100ms)",
        duration
    );
...
fn test_zk_overhead_vs_baseline() {
    ...
    assert!(
        ...
        "ZK overhead {:.2}% exceeds 50% threshold",
        overhead_percent
    );
}
