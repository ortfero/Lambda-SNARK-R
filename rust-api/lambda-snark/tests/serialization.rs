...
use lambda_snark::{Opening, Challenge, Polynomial, Commitment, LweContext};
...
fn test_opening_size_tv1() {
    ...
    assert!(bytes.len() < 1024, "Opening should be < 1KB, got {} bytes", bytes.len());
}
...
fn test_challenge_deterministic() {
    ...
    assert_eq!(bytes1, bytes2, "Challenge serialization should be deterministic");
}
...
fn test_challenge_size() {
    ...
    assert!(bytes.len() < 64, "Challenge should be < 64 bytes, got {} bytes", bytes.len());
}
...
fn test_commitment_serialization() {
    ...
    // Expected: LWE commitment for n=4096, k=2 → 8192 u64 = 65536 bytes
    // Plus bincode overhead (length prefix ~8 bytes)
    assert!(bytes.len() > 65000 && bytes.len() < 66000, 
            "Commitment size should be ~65KB, got {} bytes", bytes.len());
}
...
fn test_proof_component_sizes() {
    ...
    println!("  Commitment: {} bytes (~{}KB)", comm_bytes.len(), comm_bytes.len() / 1024);
    println!("  Challenge:  {} bytes", chal_bytes.len());
    println!("  Opening:    {} bytes", open_bytes.len());
    println!("  Total:      {} bytes (~{}KB)", 
    ...
    assert!(total_size < 100_000, "Total proof should be < 100KB for test params");
}
...
fn test_different_openings_different_bytes() {
    ...
    assert_ne!(bytes1, bytes2, "Different openings should serialize differently");
}
...
fn test_different_challenges_different_bytes() {
    ...
    assert_ne!(bytes1, bytes2, "Different challenges should serialize differently");
}
