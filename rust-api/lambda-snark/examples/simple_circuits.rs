...
pub fn square_gate() -> (R1CS, Vec<u64>) {
    ...
    // Constraint: a * a = b
    builder.add_constraint(vec![(a, 1)], vec![(a, 1)], vec![(b, 1)]);
...
pub fn fibonacci() -> (R1CS, Vec<u64>) {
    ...
    // F_2 = F_0 + F_1
    builder.add_constraint(vec![(f0, 1), (f1, 1)], vec![(one, 1)], vec![(f2, 1)]);

    // F_3 = F_1 + F_2
    builder.add_constraint(vec![(f1, 1), (f2, 1)], vec![(one, 1)], vec![(f3, 1)]);

    // F_4 = F_2 + F_3
    builder.add_constraint(vec![(f2, 1), (f3, 1)], vec![(one, 1)], vec![(f4, 1)]);

    // F_5 = F_3 + F_4
    builder.add_constraint(vec![(f3, 1), (f4, 1)], vec![(one, 1)], vec![(f5, 1)]);
