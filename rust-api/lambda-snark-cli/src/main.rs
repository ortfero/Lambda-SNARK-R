...
use lambda_snark::{prove_r1cs, verify_r1cs, CircuitBuilder, LweContext};
...
fn run_r1cs_example(seed: u64, verbose: bool) -> anyhow::Result<()> {
    ...
    println!(
        "   ✓ Circuit built: {} constraints, {} variables, modulus={}",
...
fn run_range_proof_example(seed: u64, verbose: bool) -> anyhow::Result<()> {
    ...
    // This forces b_i ∈ {0, 1}
    println!(
        "   Adding {} boolean constraints (b_i · (b_i - 1) = 0)...",
        k
    );
    ...
    for (i, &bit_var) in bit_vars.iter().enumerate() {
        ...
        // A = [b_i], B = [b_i], C = [b_i]
        builder.add_constraint(vec![(bit_var, 1)], vec![(bit_var, 1)], vec![(bit_var, 1)]);
    ...
    println!(
        "   ✓ Circuit built: {} constraints ({} boolean + 1 recomposition)",
        num_constraints, k
    );
    println!();
    ...
    println!("  - Range:         [0, 2^{}) = [0, 256)", k);
    println!(
        "  - Circuit:       {} constraints ({} boolean + 1 recomp)",
        num_constraints, k
    );
    println!(
        "  - Variables:     {} (1 constant + {} bits + 1 value)",
        r1cs.witness_size(),
        k
    );
...
fn run_benchmark(max_constraints: usize, step: usize) -> anyhow::Result<()> {
    ...
    println!(
        "   Constraint sizes: {} to {} (step: {})",
        step, max_constraints, step
    );
    ...
    if results.len() >= 2 {
        ...
        println!(
            "     Empirical exponent: {:.2} (expect ~2 for O(m²))",
            empirical_exponent
        );
        println!();
    }
...
fn run_healthcare_example(seed: u64, verbose: bool) -> anyhow::Result<()> {
    ...
    println!(
        "     - Constraints: {} R1CS equations",
        r1cs.num_constraints()
    );
    println!(
        "     - Variables: {} (including intermediate)",
        r1cs.witness_size()
    );
    ...
    println!(
        "   ✓ Proof generated in {:.2} ms",
        prove_time.as_secs_f64() * 1000.0
    );
    println!(
        "   ✓ Proof size: {} bytes (constant, independent of data)",
        proof_size
    );
    println!();
